...
import com.devsu.hackerearth.backend.account.log.LoggingUtil;
...
@ControllerAdvice
@Slf4j
public class AccountTransactionExceptionHandler extends ErrorResponse {

    @ExceptionHandler(AccountNotFoundException.class)
    public ResponseEntity<ApiErrorResponse> handleAccountNotFound(AccountNotFoundException ex,
            HttpServletRequest request) {
        LoggingUtil.logWarn(log, request, "Account not found: {}", ex.getMessage());
    ...
    public ResponseEntity<ApiErrorResponse> handleTransactionNotFound(TransactionNotFoundException ex,
            HttpServletRequest request) {
        LoggingUtil.logWarn(log, request, "Transaction not found: {}", ex.getMessage());
        return buildErrorResponse(HttpStatus.NOT_FOUND, ex, request);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleAccountInactive(AccountInactiveException ex,
            HttpServletRequest request) {
        LoggingUtil.logWarn(log, request, "Inactive account access: {}", ex.getMessage());
    ...
    public ResponseEntity<ApiErrorResponse> handleInsufficientBalance(InsufficientBalanceException ex,
            HttpServletRequest request) {
        LoggingUtil.logWarn(log, request, "Insufficient balance: {}", ex.getMessage());
        return buildErrorResponse(HttpStatus.CONFLICT, ex, request);
    }
